...
class GRU_GCN(nn.Module):
    """
    Sequential GRU followed by a two-layer batch-aware GNN.
    inputs:  (T, B, N, F)
    ...
    def forward(
        ...
        node_final = self.activate(node_seq[-1])  
        ...
        edge_index = self._create_full_edge_index(N, device=device)  
...
class GRU_GCN_classification(nn.Module):
    ...
    def forward(
        ...
        if self.agg == "max":
            ...
            pool_logits = torch.sum(logits, dim=1)     
