...
class BIOTEncoder(nn.Module):
    ...
    def forward(self, x, n_channel_offset=0, perturb=False, channel_indices=None):
        """
        ...
        for i in range(x.shape[1]):
            ...
            token_index = self.index[i + n_channel_offset] if channel_indices is None else channel_indices[i]
            channel_token_emb = (
                self.channel_tokens(token_index)
                .unsqueeze(0)
                .unsqueeze(0)
...
class BIOTClassifier(nn.Module):
    ...
    def forward(self, x, channel_indices=None):
        x = self.biot(x, channel_indices=channel_indices)
        x = self.classifier(x)
        return x
