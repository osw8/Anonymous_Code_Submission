...
def get_physionet_dataset():
    ...
    dataset = CSVFolderDataset(
                        ...
                        online_transform=transforms.Compose([
                            ...
                            transforms.Lambda(lambda x: temporal_interpolation(x, 256*6) * 1e3), 
                            transforms.To2d()
                        ]),
...
def get_TSU_dataset():
    dataset = TSUBenckmarkDataset(
            ...
            online_transform=transforms.Compose([
                ...
                transforms.ToTensor(),
                transforms.Lambda(lambda x: temporal_interpolation(x, 256*4) / 1000),
                transforms.To2d(),
            ]),
...
def get_SEED_dataset():
    dataset = SEEDDataset(
                          ...
                          online_transform=transforms.Compose([
                              ...
                              transforms.Lambda(lambda x: temporal_interpolation(x, 256*10)/1000),
                              transforms.To2d(),                          
                          ]),
                          label_transform=transforms.Compose([
...
if __name__=="__main__":
    ...
    for tag in ["PhysioNetMI", "tsu_benchmark", "seed"]: 
        if tag == "PhysioNetMI":
