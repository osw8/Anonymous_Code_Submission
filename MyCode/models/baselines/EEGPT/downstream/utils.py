...
if not os.path.exists(data_path):
    print('BCIC_2a_0_38HZ data is missing. Initializing the dataset.')
...
def select_free_gpu():  
    """Select an available GPU with sufficient memory."""  
    gpus = torch.cuda.device_count()  
    if gpus == 0:  
        return None  
    else:  
...
def callback(res):
        print('<process %s> subject %s accuracy %s' %(os.getpid(),res['sub'], str(res["accu"])))
...
def read_csv_epochs(filename, tmin, tlen, use_channels_names=use_channels_names, data_max=None, data_min=None):
    ...
    data = torch.tensor(raw.iloc[:,1:-2].values) 
